                        ...
                        <div class="card shadow-sm rounded">
                            <div class="card-body">
                                ...
                                <p><strong>Preferred Time:</strong>
                                    {{ \Carbon\Carbon::parse($serviceRequest->preferred_time)->format('d M Y, h:i A') }}</p>
                                <p><strong>Description:</strong> {{ $serviceRequest->description }}</p>
                                <p><strong>Base Price:</strong> Rs.
                                    {{ number_format($serviceRequest->service->base_price, 2) }}</p>

                                <form method="POST"
                                    action="{{ route('caregiver.acceptBasePrice', $serviceRequest->id) }}">
                                    @csrf
