    ...
    <div class="card border-0 shadow-sm mb-3" style="border-radius: 16px;">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                ...
                <div class="text-end">
                    <span class="fw-bold text-primary">Rs {{ number_format(optional($request->service)->base_price ?? 0, 0) }}</span>
                </div>
            </div>
            <p class="text-muted small mb-0 mt-2">
                <i class="bi bi-geo-alt me-1"></i> {{ $request->location }} · <i class="bi bi-calendar me-1"></i> {{ $request->preferred_time ? $request->preferred_time->format('d M Y, h:i A') : '—' }}
            </p>
            ...
            @if ($request->status === 'accepted' && $request->bookings->isNotEmpty())
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <a href="{{ route('patient.bookings.show', $booking->id) }}" class="btn btn-primary btn-sm">View booking</a>
                </div>
            @endif
        </div>
    </div>
