...
<body>
 <nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm fixed-top border-bottom border-info">
    <div class="container">
      <a class="navbar-brand d-flex align-items-center fw-bold text-info" href="{{ route('home') }}">
        <img src="{{ asset('images/logo.png') }}" alt="SewaCare Logo" class="logo-img me-2">
      </a>
      ...
      <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
        <ul class="navbar-nav">
          ...
          <li class="nav-item"><a class="nav-link nav-hover text-dark fw-semibold" href="{{ route('patient.services.index') }}">Services</a></li>
          <li class="nav-item"><a class="nav-link nav-hover text-dark fw-semibold" href="{{ route('patient.bookings.index') }}">Book</a></li>
  ...
  <section class="py-5 mt-5 bg-light position-relative overflow-hidden">
    <div class="position-absolute top-0 start-0 w-100 h-100 opacity-25" style="background: url('{{ asset('images/background.jpeg') }}') center/cover;"></div>
      ...
      <div class="row">
        ...
        <div class="col-md-4 mb-4">
          ...
          <ul class="list-unstyled small">
            ...
            <li><a href="{{ route('patient.services.index') }}" class="text-info text-decoration-none"><i class="bi bi-activity me-1"></i>Services</a></li>
            <li><a href="{{ route('patient.bookings.index') }}" class="text-info text-decoration-none"><i class="bi bi-calendar-check me-1"></i>Book</a></li>
...
<script>
  ...
  function appendMessage(sender, message) {
    ...
    chatBody.innerHTML += `<small><strong>${sender}:</strong> ${safe}</small><br>`;
    chatBody.scrollTop = chatBody.scrollHeight;
  }

  function showTyping() {
    chatBody.innerHTML += `<small id="typing"><em>Bot is typing...</em></small><br>`;
    chatBody.scrollTop = chatBody.scrollHeight;
  }
  ...
  async function sendMessage(message) {
    ...
    try {
      ...
      if (response.ok && data.response) {
        ...
        const errFromValidation = data.errors?.message ? (Array.isArray(data.errors.message) ? data.errors.message[0] : data.errors.message) : null;
        const errorMsg = data.error || data.details || data.message || errFromValidation || "Sorry, something went wrong. Please try again.";
        appendMessage("Bot", errorMsg);
      }
    } catch (error) {
      console.error(error);
      removeTyping();
      appendMessage("Bot", "Sorry, something went wrong. Check your connection and try again.");
    }
  }
